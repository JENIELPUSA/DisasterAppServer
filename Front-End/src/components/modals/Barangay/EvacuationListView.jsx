...
const EvacuationListView = ({
  ...
  const calculateStats = () => {
    const totalCapacity = evacuationData.reduce(
      (sum, evac) => sum + (evac.evacuationCapacity || 0),
      0
    );
    const totalCurrent = evacuationData.reduce(
      (sum, evac) => sum + (evac.currentEvacuation || 0),
      0
    );
    const totalAvailable = evacuationData.reduce(
      (sum, evac) => sum + (evac.availableCapacity || 0),
      0
    );
    const activeCenters = evacuationData.filter((evac) => evac.isActive).length;
    const fullCenters = evacuationData.filter(
      (evac) => evac.status === "Full"
    ).length;
  ...
  const filteredEvacuationData = evacuationData.filter(
    (evac) =>
      (evac.evacuationName || "")
        .toLowerCase()
      ...
      >
        <View
          className={`p-4 bg-white border ${
            item.isActive ? "border-gray-200" : "border-gray-300"
        ...
        >
          ...
          <View className="flex-row justify-between items-start mb-2">
            <View className="flex-1">
              ...
              <Text
                className={`text-sm mb-2 ml-11 ${
                  item.isActive ? "text-gray-500" : "text-gray-400"
                }`}
              >
          ...
          <View className="ml-11 mb-3 flex-row items-center justify-between">
            <View className="flex-row items-center">
              ...
              >
                Lat: {item.location?.latitude?.toFixed(4) || "N/A"}, Long:{" "}
          ...
          <View className="flex-row items-center">
            ...
            <View>
              <Text className="text-lg font-semibold text-gray-800">
                {evacuationData.length} Evacuation Center
                {evacuationData.length !== 1 ? "s" : ""}
              </Text>
          ...
          <View className="flex-row items-center justify-between mb-2">
            ...
            <Text className="text-cyan-800 text-sm">
              {stats.totalCapacity > 0
                ? `${((stats.totalCurrent / stats.totalCapacity) * 100).toFixed(
                    1
                  )}% occupied`
                : "No capacity data"}
            </Text>
            ...
            {stats.totalCapacity > 0 && (
              <View
                ...
                style={{
                  width: `${Math.min(
                    100,
                    (stats.totalCurrent / stats.totalCapacity) * 100
                  )}%`,
                }}
              />
            )}
        ...
        <View className="flex-row space-x-2">
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              Alert.alert(
                "Info",
                "Generate Report functionality would be implemented here"
              )
            }
            activeOpacity={0.7}
          >
